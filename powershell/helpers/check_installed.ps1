function CheckInstalled
{
    ...
    {
        $item = Get-ChildItem -LiteralPath $hive | Get-ItemProperty | Where-Object { $_.DisplayName -eq $softwareName -and ($_.DisplayVersion -eq $softwareVersion -or [string]::IsNullOrEmpty($_.DisplayVersion)) }
        if ($item)
        {
